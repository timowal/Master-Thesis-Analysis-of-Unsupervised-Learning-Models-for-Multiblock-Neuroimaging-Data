### Code for obtaining Tucker congruence and Pearson correlation coefficients for DISCO-SCA Models
...
# Function to compute Pearson correlation coefficients for DISCO-SCA models
DISCO_SCA_pearson_score <- function(model_1, model_2) {
...
# Function to compute Tucker Congruence Coefficient for DISCO-SCA models
DISCO_SCA_tucker_congruence <- function(model_1, model_2) {
  # Extract the score matrices from both DISCO-SCA models
  scores_model_1 <- model_1$scores
  scores_model_2 <- model_2$scores
  
  # Initialize a matrix to store the Tucker correlations
  ...
  for (i in 1:n_comp) {
    for (j in 1:n_comp) {
      # Extract the scores for the current components
...
# Function to loop for the multiple realisations of the simulated/contaminated data DISCO-SCA models
DISCO_SCA_score_loops <- function(n_simulation_datasets, model_directory, file_type) {
  tucker_result_list <- numeric(10)
  pearson_result_list <- numeric(10)
  
  # Loop over all combinations of uncontaminated simulated data and contaminated data DISCO-SCA models
  for (i in 1:n_simulation_datasets) {
    filename_uncontaminated <- file.path(project_directory,paste0("Results/Simulated_data/disco_sca_simulated_", i, ".rds"))
    disco_sca_model_uncontaminated <- readRDS(filename_uncontaminated)
    
    filename_contaminated <- file.path(model_directory, paste0(file_type, i, ".rds"))
    disco_sca_model_contaminated <- readRDS(filename_contaminated)
    
    current_tucker <- DISCO_SCA_tucker_congruence(disco_sca_model_uncontaminated, disco_sca_model_contaminated)
    tucker_result_list[i] <- current_tucker
    
    current_pearson <- DISCO_SCA_pearson_score(disco_sca_model_uncontaminated, disco_sca_model_contaminated)
...
scores <- DISCO_SCA_score_loops(n_simulation_datasets, model_directory, file_type)
